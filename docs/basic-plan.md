...
- 首页展示版块总览和最新帖子预览
- 按版块筛选帖子
- 页头按帖子标题关键词搜索，并跳转到 `/posts?keyword=...` 展示结果
- 全部帖子列表和版块帖子列表支持按最近回复、浏览数、回复数排序
- 帖子浏览量
- 回复数量展示
- 最近更新时间展示
...
- 管理员登录
- 版块管理：新增、修改、启用/停用，停用后可永久删除
- 帖子管理：查看、隐藏/恢复帖子，隐藏后可永久删除
- 回复状态管理：隐藏/恢复回复，隐藏后可永久删除
...
- `/posts`：全站帖子列表，展示页头标题搜索结果并支持按最近回复、浏览数、回复数排序
...
- `/admin/boards`：版块管理，列表中显示版块 ID，支持按 ID 精确搜索或按名称关键词搜索；正常版块在上方，停用版块移动到底部单独 section，并支持恢复启用和永久删除，删除版块时级联删除其下所有帖子和回复
- `/admin/posts`：帖子管理，列表中显示帖子 ID，支持按 ID 精确搜索、按帖子标题关键词搜索、按版块筛选、按状态筛选；正常帖子在上方，隐藏帖子移动到底部单独 section，并支持恢复显示和永久删除，删除帖子时级联删除其下所有回复；所属版块已停用但帖子自身仍正常时，标题用淡色删除线提示
- `/admin/replies`：回复管理，列表中显示回复 ID、帖子 ID/标题、作者和内容摘要，支持按 ID 精确搜索、按帖子标题关键词搜索、按状态筛选；正常回复在上方，隐藏回复移动到底部单独 section，并支持恢复显示和永久删除；所属帖子或版块已隐藏但回复自身仍正常时，内容节选用淡色删除线提示
...
- `parent_reply_id` 不为空时，表示这是对某条回复的回复；管理员永久删除父回复后，该字段可作为历史引用 ID 保留，用于展示“该评论已移除”。
...
可见性规则：版块停用后，其下帖子和回复不再出现在普通前台页面（包括 `/profile` 和 `/profile/:id` 的发帖与回复列表）；管理员仍可在 `/admin/posts` 和 `/admin/replies` 中查看和管理这些内容，并通过淡色删除线识别父级已隐藏但自身仍为正常状态的帖子或回复。帖子详情页中引用已隐藏或已删除的父回复时，引用内容统一显示为“该评论已移除”。

管理员永久删除规则：管理员只能在管理端靠下的停用/隐藏 section 中执行永久删除。删除版块会级联删除其下所有帖子和回复；删除帖子会级联删除其下所有回复；删除回复不会级联删除引用它的子回复，子回复保留并在引用区域显示“该评论已移除”。

现有演示库如果不是重新执行 `sql/schema.sql` 创建，需要先执行 `sql/migration-admin-hard-delete.sql`，同步级联外键和 `parent_reply_id` 历史引用规则。
...
说明：新增回复时允许携带 `parentReplyId`。为空表示直接回复帖子，不为空表示引用式回复某条回复。管理员回复接口用于在回复管理页中查看、隐藏、恢复和永久删除回复；`GET /admin/posts/{postId}/replies` 可用于查看单个帖子下的回复。
