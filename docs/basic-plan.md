...
- 管理端统计卡片：用户数、版块数、停用版块数、帖子数、隐藏帖子数、回复数、隐藏回复数
...
- `/admin`：管理员概览页，展示版块总数、停用版块数、帖子总数、隐藏帖子数等统计信息，并提供进入版块管理和帖子管理的入口
- `/admin/boards`：版块管理，列表中显示版块 ID，支持按 ID 精确搜索或按名称关键词搜索；正常版块在上方，停用版块移动到底部单独 section，并支持恢复启用
- `/admin/posts`：帖子管理，列表中显示帖子 ID，支持按 ID 精确搜索、按帖子标题关键词搜索、按版块筛选、按状态筛选；正常帖子在上方，隐藏帖子移动到底部单独 section，并支持恢复显示；回复的隐藏/恢复管理并入该页面，不单独新增回复管理页面
...
说明：新增回复时允许携带 `parentReplyId`。为空表示直接回复帖子，不为空表示引用式回复某条回复。管理员回复接口用于在帖子管理页中查看、隐藏和恢复回复，不单独新增回复管理页面。

### 管理端统计接口

- `GET /admin/dashboard/stats`

用于 `/admin` 概览卡片展示用户数、版块数、停用版块数、帖子数、隐藏帖子数、回复数、隐藏回复数。这里的 `dashboard` 只表示统计接口命名，不对应单独的 `/admin/dashboard` 页面。
